    ...
    <>
      <div className="flex justify-center items-center gap-4 mb-12">
        ...
        >
          ...
          <span
            ...
            style={{
              backgroundImage: "linear-gradient(to right, #f97316, #a855f7)",
            }}
          >
            Save 25%
          </span>
      ...
      <div className="grid md:grid-cols-3 gap-6 max-w-[960px] mx-auto px-4">
        {filteredPlans.map((plan) => {
          const isCreatorFeatured =
            plan.name.toLowerCase() === "creator" ||
            (plan.name.toLowerCase().includes("creator") &&
              !plan.name.toLowerCase().includes("pro"));

          const correspondingMonthly = plans.find(
            (p) =>
              p.name === plan.name && p.billingPeriod === billingPeriod.MONTHLY,
          );
          const monthlyPrice = correspondingMonthly?.priceAmount || 0;

          const averageMonthly = isYearly
            ? plan.priceAmount / 12
            : plan.priceAmount;
          const yearlySavings = isYearly
            ? monthlyPrice * 12 - plan.priceAmount
            : 0;
                ...
                {isYearly && yearlySavings > 0 && (
                  <>
                    ...
                    <div className="text-[#a855f7] text-[13px] font-medium tracking-tight">
                      ${averageMonthly.toFixed(2).replace(/\.00$/, "")}/mo ·
                      billed ${plan.priceAmount.toFixed(2).replace(/\.00$/, "")}
                      /yr
                    </div>
                  </>
                )}
              ...
              <div className="space-y-3 mb-8 flex-grow w-full">
                {(plan.metadata || []).map((meta, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <svg
                      className="w-5 h-5 text-[#4ade80] shrink-0 mt-0.5"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                    >
